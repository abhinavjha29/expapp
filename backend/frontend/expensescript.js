...
const page =  0 ;
const limit = 3 ;
   const response = await axios.get(`http://54.208.115.249:3800/expense/getexpense` , {headers : {"Authorization" : token} ,
  params : {
    page : page ,
    limit : limit 
  }
...
try {
  ...
  const result =  axios.post('http://54.208.115.249:3800/expense/postexpense' , user_data, {headers : {"Authorization" : token} })
  ...
  del.addEventListener('click' , async(e)=>{
    ...
    if(e.target.classList.contains('del-btn')) {
      try {
        ...
        const token =  localStorage.getItem('token') ;
  const res = await axios.delete(`http://54.208.115.249:3800/expense/deleteexpense/${e.target.id}` , {headers : {"Authorization" : token} }) 
  if (res.status==200) {
...
try {
  ...
  const token = localStorage.getItem('token') ;
  console.log("check1")
  const response = await axios.get('http://54.208.115.249:3800/premium/premiummembership' , { headers : {"Authorization" : token}})
  console.log(response) ;
  const options = {
  "key" : response.data.key_id ,
  "order_id" : response.data.order.id ,
  "handler" : async function (response) {
  const resp =   await axios.post('http://54.208.115.249:3800/premium/updatestatus', {
...
async function chekcpremium() {
  try {
    ...
    const user = await axios.get('http://54.208.115.249:3800/user/get' , { headers : {"Authorization" : token}}) ;
...
const token = localStorage.getItem('token') ;
  ...
  //   try {
  //     e.preventDefault() ;
  //     const res = await axios.get('http://54.208.115.249:3800/premfeature/leaderboard' ,{ headers : {"Authorization" : token}} )
...
try {
  e.preventDefault() ;
const token = localStorage.getItem('token') ;
const response = await axios.get('http://54.208.115.249:3800/expense/download' , { headers: {"Authorization": token}})
...
async function totalcount() {
  ...
  axios
    .get(`http://54.208.115.249:3800/expense/gettotal`, {
      headers: { authorization: token },
    })
    .then((result) => {
      let k = 3;
...
if(e.target.id) {
 const token = localStorage.getItem('token') ;
 const response = await axios.get(`http://54.208.115.249:3800/expense/getexpense?${e.target.id}` , {headers : {"Authorization" : token} }); 
 for (let i = 0; i < response.data.details.length; i++) {
  showexpensedetail(response.data.details[i]);
